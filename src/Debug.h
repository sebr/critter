...
#ifndef CRITTER_DEBUG_H
#define CRITTER_DEBUG_H
...
namespace Debug
{
    extern QMutex mutex; // defined in Critter.cpp
    extern bool enabled; // defined in Critter.cpp
    ...
    #define APP_PREFIX ""
    #else
    #define APP_PREFIX "[" DEBUG_PREFIX "]"
    #endif
    ...
    static inline QDebug fatal()   { mutex.lock(); QString ind = indent(); mutex.unlock(); return dbgstream() << qPrintable( qApp->applicationName() + ": " + ind + APP_PREFIX ); }
    ...
    public:
                ...
                : m_label( label )
        {
            if ( gettimeofday( &m_start, 0 ) == -1 )
                dbgstream() << qApp->applicationName() << ": Block - gettimeofday failed with "
                            << strerror(errno);
            ...
            dbgstream() << qPrintable( qApp->applicationName() + ": " + indent() + "BEGIN: " + label );
        ...
        ~Block()
        {
            ...
            dbgstream() << qPrintable( qApp->applicationName() + ": " + indent() + "END__: " + m_label + " - Took " + QString::number( duration, 'g', 2 ) + "s" );
            mutex.unlock();
        }
    };
